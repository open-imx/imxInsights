...
class ManifestBuilder:
    ...
    @staticmethod
    def _create_manifest_xml_imx_12() -> etree.Element:
        nsmap = {
        ...
        manifest.append(etree.Comment("This IMX 12 manifest is created (and zipped as a container) using imxInsights, see open-imx.nl for more information."))
        manifest.append(etree.Comment('Manifest and zip container currently marked as "INTENDED FOR TEST PURPOSES"!'))
        return manifest

    def _add_file_to_list(self, file_name: str, parent_element: etree.Element, file_type: str):
        file_path = self.folder_path / file_name
        if file_path.is_file():
            file_hash = hash_sha256(file_path)
            if file_type == "imspoor":
                etree.SubElement(parent_element, "ImSpoorData", attrib={"fileName": file_name, "hash": file_hash})
            elif file_type == "media":
                media_type = get_media_NIME_type(file_name)
                etree.SubElement(parent_element, "Media", attrib={
                    "fileName": file_name,
                    "mediaType": media_type,
                    "hash": file_hash
                })

    def _add_im_spoor_data_imx_12(self, manifest: etree.Element):
    ...
    def _add_media_data_imx_12(self, manifest: etree.Element):
    ...
    def _build_manifest_imx12(self) -> etree.Element:
    ...
    def save_manifest(self):
        ...
        with open(manifest_file, "wb") as f:
            ...
            f.write(etree.tostring(self.manifest, pretty_print=True, xml_declaration=True, encoding="UTF-8"))
...
if __name__ == "__main__":
    ...
    builder = ManifestBuilder(folder, Path(r"C:\Users\marti\OneDrive - ProRail BV\ENL\TVP1\test-data-km-added\output.zip"))
